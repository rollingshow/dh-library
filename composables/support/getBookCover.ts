export const useGetBookCover = (isbn) => {
    let thumbnailURL = ''
    ...
    if (!isbn) {
        ...
        let openLibraryData: any = $fetch(`https://openlibrary.org/isbn/${isbn}.json`, {
            method: 'GET'
        })
        ...
        if (thumbnailURL === '') {
            try {
                let googleBooksData: any = $fetch(`https://www.googleapis.com/books/v1/volumes?q=isbn:${isbn}`, {
                    method: 'GET'
                })
        ...
        if (thumbnailURL === '') {
            try {
                ...
                if (openLibraryData.title) {
                    ...
                    let yandexSearchData: any = $fetch(`https://yandex.com/images/search?text=${title}&img_url=${title}&rpt=imageview`, {
                        method: 'GET'
                    })
